import { AtpAgent, type AtpAgentLoginOpts, type ComAtprotoRepoDeleteRecord, type ComAtprotoRepoPutRecord } from '@atproto/api'

import { type HandleCache } from './handle'
...
export class AtpApi implements Atp {
  ...
  constructor (
    ...
    private readonly handleCache: HandleCache | null
  ) { }
  ...
  async resolveHandle (handle: string): Promise<string> {
    ...
    if (this.handleCache !== null) {
      return await this.handleCache.fetchDid(handle, resolve)
    } else {
  ...
  async resolveDid (did: string): Promise<string> {
    ...
    if (this.handleCache !== null) {
      return await this.handleCache.fetchHandle(did, resolve)
    } else {
      return await resolve()
    }
  }
