    ...
    describe("fetchDid", () => {
        context("cache miss", () => {
            it('should return the value from the handler', async () => {
                const cache = await HandleCache.create({ max: 10, ttl: 5*60*60*1000 })
        ...
        describe("ttl purge on write", () => {
            it("should remove records that are older than ttl", async () => {
                const cache = await HandleCache.create({ max: 10, ttl: 10 /*ms*/ })
        ...
        describe("max records purge", () => {
            ...
            it("should remove the oldest records", async () => {
                const cache = await HandleCache.create({ max: 1, ttl: 5*60*60*1000 })
